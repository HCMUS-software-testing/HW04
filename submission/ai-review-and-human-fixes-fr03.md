...
| --- | --- | --- |
| **Locator quá chung chung và thiếu chuẩn accessibility:** AI dùng `page.locator('input[type="text"]').first()` cho cả ô nhập Email (Bước 1) và ô nhập OTP (Bước 2). | Sửa sang Playwright Locators chuẩn: `page.getByPlaceholder(/email/i)` cho email và `page.getByPlaceholder(/otp|mã/i)` cho OTP. | AI suy đoán DOM tiêu chuẩn chung, không trực tiếp inspect DOM thực tế của ứng dụng SUT. |
